...
public class UserResource {
    ...
    public Response putUser(JSONObject jsonEntity) throws JSONException {
      ...
      System.out.println("UserResource.putUser(): " +jsonUserid+"=="+userid);
      ...
      if (newRecord) {
            TransactionManager.manage(new Transactional(em) { public void transact() {
              System.out.println("UserResource.putUser() persist record");
                em.persist(userEntity);
            }});
            return Response.created(uriInfo.getAbsolutePath()).build();
        } else {
            TransactionManager.manage(new Transactional(em) { public void transact() {
              System.out.println("UserResource.putUser() merge record");
                em.merge(userEntity);
            }});
